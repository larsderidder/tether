"""Persistent state management for session↔topic mapping."""
...
class StateManager:
    ...
    Stores mappings as JSON to allow the bridge to resume after restarts,
    preserving the connection between sessions and their forum topics.
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.info("Loaded %d session mappings from %s", len(self._mappings), self._path)
        except Exception:
    ...
    def get_topic_for_session(self, session_id: str) -> int | None:
        """Get the topic ID for a session, if one exists."""
    ...
    def get_session_for_topic(self, topic_id: int) -> str | None:
        """Get the session ID for a topic, if one exists."""
